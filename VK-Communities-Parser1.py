...
def get_posts(group_id, access_token, count=100):
    ...
    while count > 0:
        ...
        response = requests.get(base_url, params=params)
        data = response.json()
        """data['response']['items']['date'] = datetime.fromtimestamp(data['response']['items']['date'])
        ...
        if 'response' in data and 'items' in data['response']:
            ...
            posts.extend([post['text'].replace('\n', ' ')+(datetime.fromtimestamp(post['date']).strftime("%m/%d/%Y, %H:%M:%S")) for post in items if post['text']])
            count -= len(items)
            offset += len(items)
        else:
...
group_domain = 'powermorning_sesc'  # Replace with the actual group ID+
...
filename = f'{group_domain}draft2.txt'
